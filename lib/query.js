...
const list = async (options) => {
  ...
  try {
    ...
    do {
      let data = await queryClient.listScheduledQueries(params).promise();
      let scheduledQueryList = data.ScheduledQueries;
...
const setOnOff = async (options) => {
  return new Promise((resolve, reject) => {
    ...
    queryClient.updateScheduledQuery(params).promise().then((data) => {
      resolve(`Successfully updated scheduled query state to ${params.State}`)
    }, (err) => {
      const error = `Update Scheduled Query failed.`
      reject(error)
    })
