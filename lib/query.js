...
const get = async (options) => {
  return new Promise((resolve, reject) => {
    ...
    const table = options && options.table ? options.table : process.env.AWS_TABLE_NAME
    const nextToken = options && options.nextToken ? options.nextToken : null

    const params = {
      QueryString: options && options.query ? options.query : `SELECT * FROM "${db}"."${table}"`
    }
