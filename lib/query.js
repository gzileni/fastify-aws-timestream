...
const _parseRow = (columnInfo, row) => {
  ...
  for (let i=0; i < row.Data.length; i++) {
    let item = _parseDatum(columnInfo[i], row.Data[i]);
    rowOutput.push(item)
  }
...
const get = async (options) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const tb = options && options.table ? options.table : process.env.AWS_TABLE_NAME;

    const params = {
      QueryString: options && options.query ? options.query : `SELECT * FROM ${db}.${tb}`
    };
  ...
  */
  ...
  return new Promise((resolve, reject) => {
    ...
    const timestreamConfiguration = {
      ...
      TableName: options && options.toTable ? options.toTable : `${process.env.AWS_TABLE_NAME}_${uuidv4()}`,
      TimeColumn: options.timeColumn,
      DimensionMappings: options.dimensions,
      MultiMeasureMappings: options.mappings
    };
    ...
    const createScheduledQueryRequest = {
      ...
      ScheduledConfiguration: {
        ScheduleExpression: options.scheduled_cron ? options.scheduled_cron : 'cron(0/2 * * * ? *)'
      },
      TargetConfiguration: {
    ...
    if (options.topicArn) {
      createScheduledQueryRequest["NotificationConfiguration"] = {
        SnsConfiguration: {
    ...
    if (options.roleArn) {
      createScheduledQueryRequest["ScheduledQueryExecutionRoleArn"] = options.roleArn
    };

    if (options.s3ErrorReportBucketName) {
      createScheduledQueryRequest["ErrorReportConfiguration"] = {
        S3Configuration: {
